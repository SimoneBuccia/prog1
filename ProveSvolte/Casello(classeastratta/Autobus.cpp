...
Autobus::Autobus(const char * t, const char * a, const char * ti, const int po, const char * des, const int mas) : Autovettura(t,a,ti,po) {
	Dest = new char[strlen(des)+1];
	strcpy(Dest, des);
	Massa = mas;
}

Autobus::Autobus(const Autobus & a) : Autovettura(a) {
...
ostream & operator<<(ostream & out, const Autobus & a) {
	out << (Autovettura&)a << a.getDest() << ' ' << a.getMassa() << ' ';
	return out;
}
